    ...
    <AuthenticationCard>
        ...
        <div class="mb-4 text-sm text-gray-600 dark:text-gray-400">
            <template v-if="! recovery">
                {{$t('two_factor_challenge.phone')}}
            </template>

            <template v-else>
                {{$t('two_factor_challenge.codes')}}
            </template>
        </div>
        ...
        <form @submit.prevent="submit">
            ...
            <div v-else>
                <InputLabel for="recovery_code"
                            value="$t('two_factor_challenge.recovery_code')" />
                <TextInput
                    id="recovery_code"
            ...
            <div class="flex items-center justify-end mt-4">
                <button type="button"
                    ...
                    <template v-if="! recovery">
                        {{$t('two_factor_challenge.use_recovery')}}
                    </template>

                    <template v-else>
                        {{$t('two_factor_challenge.use_phone')}}
                    </template>
                </button>
